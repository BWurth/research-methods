...
The histogram will display the distribution of the `tpstress` variable (@fig-hist-tpstress), showing the frequency of scores within each bin. This can help answer questions like:

-  Are most participants experiencing low, moderate, or high stress?
-  Does the data appear skewed or normally distributed?

![Histogram of `tpstress`](images/c2_histogram_tpstress.jpg){#fig-hist-tpstress width="80%" fig-align="center" alt="Histogram of `tpstress`"}
...
The panelled (@fig-hist-tpstress_gender) and stacked histograms (@fig-hist-tpstress_gender) show the distribution of perceived stress scores (`tpstress`) for males and females:
...
![Panelled histograms for `tpstress` by `sex`](images/c2_histogram_tpstress_gender.jpg){#fig-hist-tpstress_gender width="80%" fig-align="center" alt="Panelled histograms for `tpstress` by `sex`"}

![Stacked histograms for `tpstress` by `sex`](images/c2_histogram_tpstress_gender_stack.jpg){#fig-hist-tpstress_gender_stack width="80%" fig-align="center" alt="Stacked histograms for `tpstress` by `sex`"}
...
Running the above code will produce a simple bar chart showing the distribution of males and females in the dataset (@fig-bar_gender). The y-axis represents the count of participants in each gender category.

![Simple bar chart for `sex`](images/c3_bar_gender.jpg){#fig-bar_gender width="80%" fig-align="center" alt="Simple bar chart for `sex`"}
...
The clustered bar chart (@fig-bar_gender_age) allows us to compare:
...
![Clustered bar chart for `tpstress` by `sex`](images/c3_bar_gender_age.jpg){#fig-bar_gender_age width="80%" fig-align="center" alt="Clustered bar chart for `tpstress` by `sex`"}
...
In the **simple line graph** (@fig-line-stress-age), the x-axis represents age groups (`agegp5`), and the y-axis shows the mean perceived stress scores (`tpstress`). The points connected by the line indicate how stress levels change across age groups.
...
![Simple line graph for age groups (`agegp5`) and `tpstress`](images/c4_line_stress_age.jpg){#fig-line-stress-age width="80%" fig-align="center" alt="Simple line graph for age groups (`agegp5`) and `tpstress`"}
...
The **multi-line graph** adds a layer of complexity by splitting the line into separate paths for males and females (@fig-line-stress-age-gender). Each gender has its own line, distinguished by color. This allows for a direct comparison of stress levels between genders across age groups.
...
![Multi-line graph for age groups (`agegp5`) and `tpstress` grouped by `sex`](images/c4_line_stress_age_gender.jpg){#fig-line-stress-age-gender width="80%" fig-align="center" alt="Multi-line graph for age groups (`agegp5`) and `tpstress` grouped by `sex`"}
...
The simple scatterplot shows the relationship between `tpstress` (perceived stress) and `tpcoiss` (coping strategies) across all participants (@fig-scatter-stress-control). When examining a scatterplot, there are a few key points to consider.

- Direction of the relationship:
  - A **negative trend** (points sloping downward) as shown in @fig-scatter-stress-control suggests that higher coping strategies are associated with lower perceived stress.
...
![Simple scatterplot for `tpstress` and `tpcoiss`](images/c5_scatter_stress_control.jpg){#fig-scatter-stress-control width="80%" fig-align="center" alt="Simple scatterplot for `tpstress` and `tpcoiss`"}
...
The grouped scatterplot adds a layer of complexity by differentiating the points based on `sex` (males and females) using color coding (@fig-scatter-stress-control-gender), which allows for subgroup comparisons. When examining a grouped scatterplot, there are a few key points to consider.
...
![Grouped scatterplot for `tpstress` and `tpcoiss` by `sex`](images/c5_scatter_stress_control_gender.jpg){#fig-scatter-stress-control-gender width="80%" fig-align="center" alt="Grouped scatterplot for `tpstress` and `tpcoiss` by `sex`"}
...
The labeled scatterplot builds on the grouped scatterplot by adding participant identifiers (`id`) as point labels (@fig-scatter-stress-control-gender-id). This is useful for identifying specific data points and exploring individual-level patterns. There are a few key points to consider when analyzing labeled scatterplots.
...
![Grouped scatterplot for `tpstress` and `tpcoiss` by `sex` with point labels](images/c5_scatter_stress_control_gender_id.jpg){#fig-scatter-stress-control-gender-id width="80%" fig-align="center" alt="Grouped scatterplot for `tpstress` and `tpcoiss` by `sex` with point labels"}
